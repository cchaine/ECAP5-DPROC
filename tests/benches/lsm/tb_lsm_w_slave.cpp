...
#include "Vtb_lsm_w_slave.h"
#include "Vtb_lsm_w_slave_tb_lsm_w_slave.h"
#include "Vtb_lsm_w_slave_lsm.h"
#include "Vtb_lsm_w_slave_ecap5_dproc_pkg.h"
...
class TB_Lsm_w_slave : public Testbench<Vtb_lsm_w_slave> {
public:
  void reset() {
    this->_nop();
    ...
    Testbench<Vtb_lsm_w_slave>::reset();
  }
...
void tb_lsm_w_slave_no_stall_lw(TB_Lsm_w_slave * tb) {
  Vtb_lsm_w_slave * core = tb->core;
  ...
  tb->check(COND_wishbone,      (core->tb_lsm_w_slave->wb_adr_o  ==  addr) &&
                                (core->tb_lsm_w_slave->wb_we_o   ==  0)    &&
                                (core->tb_lsm_w_slave->wb_sel_o  ==  0xF)  &&
                                (core->tb_lsm_w_slave->wb_stb_o  ==  1)    &&
                                (core->tb_lsm_w_slave->wb_cyc_o  ==  1));  
  ...
  tb->check(COND_input_ready,   (core->input_ready_o             ==  0));
  tb->check(COND_wishbone,      (core->tb_lsm_w_slave->wb_stb_o  ==  0)    &&
                                (core->tb_lsm_w_slave->wb_ack_i  ==  1) &&
                                (core->tb_lsm_w_slave->wb_cyc_o  ==  1));  
  tb->check(COND_register,      (core->reg_write_o               ==  1)    &&
                                (core->reg_addr_o                ==  reg_addr));  
  tb->check(COND_output_valid,  (core->output_valid_o            ==  0));
  ...
  tb->check(COND_input_ready,   (core->input_ready_o             ==  0));
  tb->check(COND_wishbone,      (core->tb_lsm_w_slave->wb_stb_o  ==  0)    &&
                                (core->tb_lsm_w_slave->wb_ack_i  ==  0) &&
                                (core->tb_lsm_w_slave->wb_cyc_o  ==  1));  
  tb->check(COND_register,      (core->reg_write_o               ==  1)    &&
                                (core->reg_addr_o                ==  reg_addr) &&
  ...
  tb->check(COND_wishbone,      (core->tb_lsm_w_slave->wb_stb_o  ==  0)    &&
                                (core->tb_lsm_w_slave->wb_ack_i  ==  0) &&
                                (core->tb_lsm_w_slave->wb_cyc_o  ==  0));  
  tb->check(COND_register,      (core->reg_write_o               ==  1)        &&
                                (core->reg_addr_o                ==  reg_addr) &&
                                (core->reg_data_o                ==  data)); 
...
void tb_lsm_w_slave_no_stall_sw(TB_Lsm_w_slave * tb) {
  ...
  tb->check(COND_input_ready,   (core->input_ready_o             ==  0));
  tb->check(COND_wishbone,      (core->tb_lsm_w_slave->wb_adr_o  ==  addr) &&
                                (core->tb_lsm_w_slave->wb_dat_o  ==  data) &&
                                (core->tb_lsm_w_slave->wb_we_o   ==  1)    &&
                                (core->tb_lsm_w_slave->wb_sel_o  ==  0xF)  &&
                                (core->tb_lsm_w_slave->wb_stb_o  ==  1)    &&
                                (core->tb_lsm_w_slave->wb_cyc_o  ==  1));  
  ...
  CHECK("tb_lsm_w_slave.no_stall.SW_01",
      tb->conditions[COND_input_ready],
      "Failed to implement the input_ready_o signal", tb->err_cycles[COND_input_ready]);

  CHECK("tb_lsm_w_slave.no_stall.SW_02",
      tb->conditions[COND_wishbone],
      "Failed to implement the wishbone protocol", tb->err_cycles[COND_wishbone]);

  CHECK("tb_lsm_w_slave.no_stall.SW_03",
      tb->conditions[COND_register],
      "Failed to implement the register protocol", tb->err_cycles[COND_register]);

  CHECK("tb_lsm_w_slave.no_stall.SW_04",
      tb->conditions[COND_output_valid],
      "Failed to implement the output_valid_o signal", tb->err_cycles[COND_output_valid]);
}
...
int main(int argc, char ** argv, char ** env) {
  ...
  TB_Lsm_w_slave * tb = new TB_Lsm_w_slave;
  tb->open_trace("waves/lsm_w_slave.vcd");
  tb->open_testdata("testdata/lsm_w_slave.csv");
