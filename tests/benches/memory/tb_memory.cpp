...
#include "testbench.h"
#include "Vtb_memory_ecap5_dproc_pkg.h"
...
void tb_memory_port1_write(TB_Memory * tb) {
  ...
  CHECK("tb_memory.port1_write.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.port1_write.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.port1_write.03",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.port1_write.04",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_port2_read(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.port2_read.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.port2_read.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.port2_read.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.port2_read.04",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_port2_write(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.port2_write.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.port2_write.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.port2_write.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.port2_write.04",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_two_during_one(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.two_during_one.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.two_during_one.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.two_during_one.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.two_during_one.04",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.two_during_one.05",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_one_during_two(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.one_during_two.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.one_during_two.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.one_during_two.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.one_during_two.04",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.one_during_two.05",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_priority(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.priority.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.priority.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.priority.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.priority.04",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.priority.05",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_master_stall_s1(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.master_stall_s1.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.master_stall_s1.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.master_stall_s1.03",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.master_stall_s1.04",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_master_stall_s2(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.master_stall_s2.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.master_stall_s2.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.master_stall_s2.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.master_stall_s2.04",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}

void tb_memory_back_to_back(TB_Memory * tb) {
  Vtb_memory * core = tb->core;
  ...
  CHECK("tb_memory.back_to_back.01",
      tb->conditions[COND_s1_stall],
      "Failed to implement s1 stalling", tb->err_cycles[COND_s1_stall]);
  
  CHECK("tb_memory.back_to_back.02",
      tb->conditions[COND_s2_stall],
      "Failed to implement s2 stalling", tb->err_cycles[COND_s2_stall]);

  CHECK("tb_memory.back_to_back.03",
      tb->conditions[COND_s1_ack],
      "Failed to block s1 acknowledge", tb->err_cycles[COND_s1_ack]);

  CHECK("tb_memory.back_to_back.04",
      tb->conditions[COND_s2_ack],
      "Failed to block s2 acknowledge", tb->err_cycles[COND_s2_ack]);

  CHECK("tb_memory.back_to_back.05",
      tb->conditions[COND_m_wb],
      "Failed to implement master muxing", tb->err_cycles[COND_m_wb]);
}
...
int main(int argc, char ** argv, char ** env) {
  ...
  TB_Memory * tb = new TB_Memory;
  tb->open_trace("waves/memory.vcd");
  tb->open_testdata("testdata/memory.csv");
  ...
  tb_memory_port2_read(tb);
  tb_memory_port2_write(tb);

  tb_memory_two_during_one(tb);
  tb_memory_one_during_two(tb);

  tb_memory_priority(tb);

  tb_memory_master_stall_s1(tb);
  tb_memory_master_stall_s2(tb);

  tb_memory_back_to_back(tb);
