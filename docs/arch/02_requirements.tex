...
\begin{content}
  Only one immediate value can be encoded in one instruction. The value can be reconstructed from fragments of the following format : \texttt{imm[x]} representing the \texttt{x}\textsuperscript{th} bit or \texttt{imm[x:y]} representing bits from the \texttt{x}\textsuperscript{th} to the \texttt{y}\textsuperscript{th} both included.
\end{content}
