\section{Execute Module}
  ...
  \subsection{Specification}
    ...
    \subsubsection{State machine}
      ...
      \begin{figure}[H]
          \centering
          \input{arch/figures/10_exm-state-diagram}
          \caption{State diagram of the execute module}
          \label{fig:exm-state-diagram}
        \end{figure}

        \req{D\_EXM\_STATE\_01}{
            The execute module shall be in the \texttt{NORMAL (0)} state when \texttt{opcode\_i} is not \texttt{LOAD} or \texttt{STORE}.
          }

        \req{D\_EXM\_STATE\_02}{
            The execute module shall be in the \texttt{REQUEST (1)} state when \texttt{opcode\_i} is either \texttt{LOAD} or \texttt{STORE} and the memory request is successfully sent.
          }

        \req{D\_EXM\_STATE\_03}{
            The execute module shall be in the \texttt{MEMORY\_WAIT (2)} state when \texttt{opcode\_i} is either \texttt{LOAD} or \texttt{STORE} and the memory request has been sent but the response has not been received.
          }

        \req{D\_EXM\_STATE\_04}{
            The execute module shall be in the \texttt{DONE (3)} state when \texttt{opcode\_i} is either \texttt{LOAD} or \texttt{STORE} and the memory request response is successfully received.
          }

        \req{D\_EXM\_STATE\_05}{
            The execute module shall be in the \texttt{MEMORY\_STALL (4)} state when \texttt{opcode\_i} is either \texttt{LOAD} or \texttt{STORE} and the memory request is ready while the memory interface is stalled.
          }
    ...
    \subsubsection{Input registering}
      ...
      \req{D\_EXM\_INPUT\_REGISTER\_01}{
          On the rising edge of \texttt{clk\_i} when \texttt{input\_ready\_o} is asserted, \eventref{EV\_EXM\_INPUT\_REGISTER\_01} shall be triggered.
        }

      \req{D\_EXM\_INPUT\_BUBBLE\_01}{
          The registered input values shall be overriden with the values provided in table \ref{tab:exm-bubble-input} when \eventref{EV\_EXM\_INPUT\_REGISTER\_01} is triggered and \texttt{input\_valid\_o} is deasserted.
        }

      \req{D\_EXM\_INPUT\_BUBBLE\_02}{
          The registered input values shall be overriden with the values provided in table \ref{tab:exm-bubble-input} on the rising edge of \texttt{clk\_i} when \texttt{rst\_i} is asserted.
        }

      \begin{table}[H]
          \centering
          \input{arch/tables/10_exm-bubble-input.tex}
      ...
      \req{D\_EXM\_INPUT\_READY\_01}{
          The \texttt{input\_ready\_o} signal shall be asserted on the rising edge of \texttt{clk\_i} when in the \texttt{NORMAL} state. It shall be deasserted otherwise.
        }

    \subsubsection{Load triggering}
    ...
    \subsubsection{Store triggering}

      \req{D\_EXM\_STORE\_TRIGGER\_01}{
          A one byte wishbone write cycle shall be triggered on the rising edge of \texttt{clk\_i} following the triggering of \eventref{EV\_EXM\_INPUT\_REGISTER\_01} when \texttt{opcode\_i} is \texttt{STORE} and \texttt{func3\_i} is \texttt{SB-FUNC3}.
        }

      \req{D\_EXM\_STORE\_TRIGGER\_02}{
          A two byte wishbone write cycle shall be triggered on the rising edge of \texttt{clk\_i} following the triggering of \eventref{EV\_EXM\_INPUT\_REGISTER\_01} when \texttt{opcode\_i} is \texttt{STORE} and \texttt{func3\_i} is \texttt{SH-FUNC3}.
        }

      \req{D\_EXM\_STORE\_TRIGGER\_03}{
          A four byte wishbone write cycle shall be triggered on the rising edge of \texttt{clk\_i} following the triggering of \eventref{EV\_EXM\_INPUT\_REGISTER\_01} when \texttt{opcode\_i} is \texttt{STORE} and \texttt{func3\_i} is \texttt{SW-FUNC3}.
        }
    ...
    \subsubsection{Output}
      ...
      \req{D\_EXM\_OUTPUT\_DATA\_01}{
          The value of the \texttt{result\_o} shall be set to the value specified in table \ref{tab:exm-result-output} considering the values of \texttt{opcode\_i}, \texttt{func3\_i} and \texttt{func7\_i} on the rising edge of \texttt{clk\_i} following the triggering of \eventref{EV\_EXM\_INPUT\_REGISTER\_01}.
        }

      \begin{table}[H]
          \centering
          \input{arch/tables/10_exm-result-output.tex}
          \caption{Table of values for the \texttt{result\_o} output considering \texttt{opcode\_i}, \texttt{func3\_i} and \texttt{func7\_i}}
          \label{tab:exm-result-output}
        \end{table}

      \begin{content}
          The value of \texttt{result\_o} is unspecified for any combinaison of \texttt{opcode\_i}, \texttt{func3\_i} and \texttt{func7\_i} which are not present in \ref{tab:exm-result-output}.
        \end{content}
      ...
      \begin{table}[H]
          ...
          \caption{Table of values for the \texttt{branch\_o} output considering \texttt{opcode\_i}, \texttt{func3\_i} and \texttt{func7\_i}}
      ...
      \begin{table}[H]
          ...
          \caption{Table of values for the \texttt{boffset\_o} output considering \texttt{opcode\_i}, \texttt{func3\_i} and \texttt{func7\_i}}
    ...
    \subsubsection{Output handshake}
      ...
      \req{D\_EXM\_OUTPUT\_HANDSHAKE\_01}{
          The \texttt{output\_valid\_o} output shall be asserted on the rising edge of \texttt{clk\_i} following \eventref{EV\_EXM\_INPUT\_REGISTER\_01} being triggered when \texttt{opcode\_i} is one of the following : \texttt{LUI}, \texttt{AUIPC}, \texttt{JAL}, \texttt{JALR}, \texttt{OP}, \texttt{OP-IMM} and \texttt{BRANCH}.
        }

      \req{D\_EXM\_OUTPUT\_HANDSHAKE\_02}{
          The signal \texttt{output\_valid\_o} shall be deasserted on the rising edge of \texttt{clk\_i} following \eventref{EV\_EXM\_INPUT\_REGISTER\_01} being triggered when \texttt{opcode\_i} is either \texttt{LOAD} or \texttt{STORE}.
        }

      \req{D\_EXM\_OUTPUT\_HANDSHAKE\_03}{
          The signal \texttt{output\_valid\_o} shall be asserted on the rising edge of \texttt{clk\_i} following the assertion of \texttt{wb\_ack\_i} after \eventref{EV\_EXM\_INPUT\_REGISTER\_01} was triggered and \texttt{opcode\_i} is \texttt{LOAD} or \texttt{STORE}.
        }

      \req{D\_EXM\_OUTPUT\_HANDSHAKE\_04}{
          When \texttt{output\_valid\_o} is asserted, the execute module shall hold the value of the following signals until the rising edge of \texttt{clk\_i} following the assertion of \texttt{output\_ready\_i} : \texttt{result\_write\_o}, \texttt{result\_addr\_o}, \texttt{result\_o}, \texttt{branch\_o} and \texttt{boffset\_o}.
        }
